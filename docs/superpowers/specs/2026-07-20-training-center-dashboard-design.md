...
| Hosting | **Provider not chosen.** The requirement is a root Linux VPS | **This row states a capability, not a vendor, because only the capability constrains the code.** Four things must be possible: a supervised long-lived queue worker, Redis, a one-minute cron entry for the scheduler, and SSH deploys — plus a backup destination outside the server's own failure domain. Shared and cPanel hosting supply none of the first three; without them receipts, report PDFs and exports stop being asynchronous and the queued-export UI describes behaviour the server cannot produce. Candidates include Hetzner or DigitalOcean, and a local Libyan host such as Libyan Spider — Filament round-trips to the server on most interactions, so a locally hosted server is felt on every click. A management panel (Ploi, Forge) is **orthogonal to the provider**: it configures any Ubuntu box it has SSH root on. Roughly $15–25/month. |
...
| Student portal | `/portal` | Filament panel | Students (phase 3) |
...
- **Throttling:** provided by Filament's own login page, which calls `rateLimit(5)` — 5 attempts per minute, keyed per component and IP. This spec originally called for email+IP keying via a Laravel named limiter; that was written before it was known Filament self-throttles. A named `login` limiter was implemented in P1-T03 and then **removed**, because nothing referenced `throttle:login` — there are no Laravel auth routes in this app — and a registered-but-unreferenced limiter reads as an active control while protecting nothing. Phase 3 did NOT add one, and the reason is worth keeping: the student portal shipped as a second Filament panel, so it introduced no Laravel auth routes for a limiter to sit on, and that panel self-throttles at `rateLimit(5)` exactly as the admin one does. The named limiter phase 3 DID introduce is `certificate-verification`, registered in `AppServiceProvider` and applied by `routes/web.php` to the three public verifier routes — referenced by real routes, which is the property whose absence got the `login` limiter deleted.
- **Forced password change is a containment gate, not an authorization boundary.** The `ForcePasswordChange` middleware is registered **persistently** (P1-T15, security finding 5), so it runs on Livewire update requests as well as page loads. *This line previously claimed the opposite* — that it was non-persistent, and that it had to be, or the save request would redirect itself and the flow would be an inescapable trap. That argument was self-refuting and the review disproved it: the guard exempts the password-change route, and Livewire evaluates persistent middleware against the route the snapshot came from, so the form's own save was never at risk. What the non-persistent registration actually did was let a flagged user keep driving every other Livewire component on the panel.

  The guard is now component-aware (G1-U3): on Livewire's update endpoint it exempts the password-change route only when every component in the posted payload resolves to `PasswordChange`, and the page itself renders no panel chrome so there is almost nothing left to co-render. It contains a flagged session to the password form; it does not confer or withhold any permission. Anything that must be a genuine authorization boundary still belongs in a policy, not here.
...
The public verifier is an exact-reference lookup such as `/verify/certificates/{reference}`; there is no browsable certificate register, partial search, or autocomplete. Public exact-reference verification does not grant access to the internal register and is separate from the permission matrix above. It is rate-limited and displays only the certificate status, printed student name, course name, completion date, issue date, center confirmation, and — for a revoked certificate only — the date it was revoked. Phase 3 built this as a closed, named projection rather than a field count: the revocation date was added during P3-T08 because the phase-3 design requires a revoked certificate to state "revoked on <date>", and the guarantee that matters is that the list is deliberate and cannot grow by accident, not that it has any particular length. It never exposes date of birth, national ID, contact details, portal account data, or financial information.
...
**The gate has two supported environments (P35-T14).** Windows is where development, the Git hooks and the pre-push gate run. A Linux target in Docker (`docker/dev-linux/`, runbook `docs/LINUX-DEV-TARGET.md`) exists for work Windows PHP cannot host or should not stand in for: Horizon, which needs `pcntl` and `posix` — extensions with no Windows build at all — and the load and stress runs, which measure the Redis queue and Linux platform production will run. CI on Ubuntu remains the arbiter when the two disagree: a change is green when CI is green.
...
- Queue workers use Laravel's **Redis queue connection in production** (`QUEUE_CONNECTION=redis`) and run as a **supervised long-lived process** — restarted when one dies, because a worker that exits silently stops generating receipts and exports with no visible symptom. A management panel supervises them for you; a hand-configured server needs systemd or supervisor. A development machine without Redis may explicitly use `QUEUE_CONNECTION=database`; that is a local fallback, not a production topology. Sessions remain database-backed and cache remains database-backed — moving either to Redis is a separate operational decision, not implied by the queue requirement.
- Laravel Horizon owns that supervised worker topology: one auto-balanced supervisor consumes the Redis `default` queue, scales from one process to at most ten in production, and is restarted by the host process supervisor after deploys or failures. Horizon emits a mail alert to `HORIZON_ALERT_EMAIL` when `redis:default` has waited **60 seconds**, and the scheduler records Horizon metrics every five minutes. Production refuses to boot when the recipient is absent or invalid, or when the selected mail transport is the non-delivering `log` or `array` transport. Deployment still requires configuring a real mail provider and verifying receipt through that provider; application validation cannot guarantee delivery. The dashboard uses a separate `viewHorizon` gate requiring an active account with `view_any_activity`; the package's local-environment bypass remains development-only.
- Laravel Pulse records slow queries and runtime health in production using database storage with direct storage ingestion; it has no Redis ingest dependency. Every enabled first-party recorder uses a sample rate of `1`. Slow queries are recorded at **500 ms**; slow requests, jobs, and outgoing requests at **1,000 ms**. Both Pulse storage and ingest rows are retained for **7 days**. Its dashboard is separately protected by the `viewPulse` gate, which requires an active account with the existing `view_any_activity` permission because it exposes query text and user activity. The server-health recorder also requires `php artisan pulse:check` as a supervised long-lived process beside the queue worker; without it, the CPU, memory, and disk card has no data.
