...
- Laravel Pulse records slow queries and runtime health in production using database storage with direct storage ingestion; it has no Redis ingest dependency. Every enabled first-party recorder uses a sample rate of `1`. Slow queries are recorded at **500 ms**; slow requests, jobs, and outgoing requests at **1,000 ms**. Both Pulse storage and ingest rows are retained for **7 days**. Its dashboard is separately protected by the `viewPulse` gate, which requires the existing `view_any_activity` permission because it exposes query text and user activity.
