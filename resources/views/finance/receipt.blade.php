...
<body>
    ...
    <table>
        ...
        <tr><th>{{ __('receipt.student') }}</th><td>{{ $context['student_code'] }} — {{ $context['student_name'] }}</td></tr>
        ...
        @foreach ($tenders as $tender)
            <tr><th>{{ __('receipt.tender') }}: {{ $tender['label'] }}</th><td class="amount">{{ __('receipt.amount_lyd', ['amount' => $tender['amount']]) }}</td></tr>
        @endforeach
        <tr><th>{{ __('receipt.remaining_balance') }}</th><td class="amount">{{ __('receipt.amount_lyd', ['amount' => $remainingBalance]) }}</td></tr>
        <tr><th>{{ __('receipt.payment_date') }}</th><td>{{ $payment->received_at->format('Y-m-d H:i') }}</td></tr>
